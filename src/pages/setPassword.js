  ...
  useEffect(() => {
    ...
    const verifyToken = async () => {
      try {
        ...
        setTokenInvalid(true); // only case resend should show
      }
    };
  ...
  const handleSetPassword = async (e) => {
    ...
    try {
      const res = await axios.post(
        `${API_BASE_URL}/api/auth/set-password/${token}`,
        { password }
      );
      ...
      if (err.response?.status === 410 || err.response?.status === 404) {
        setTokenInvalid(true); // show resend only if truly invalid/expired
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-900 p-4">
      ...
      >
        ...
        {/* Only show password form if token is valid */}
        {!tokenInvalid && (
          <>
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">
                Minimum 8 chars, include uppercase, lowercase,
                number & special char
              </p>
            </div>
        ...
        {/* Show resend ONLY if token invalid */}
        {tokenInvalid && (
